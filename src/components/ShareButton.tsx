...
export default function ShareButton({ url, title, description, prefilledMessage }: ShareButtonProps) {
  ...
  const shareText = prefilledMessage || title;
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title,
          text: prefilledMessage || description || title,
          url,
        });
      } catch {
        // User cancelled or share failed; open fallback dropdown
        setIsOpen(true);
      }
    } else {
      setIsOpen(!isOpen);
    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          />
          <div className="absolute right-0 top-full mt-2 z-50 w-52 bg-white rounded-xl shadow-lg border border-gray-100 py-2 animate-fade-in-up">
            {shareLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
